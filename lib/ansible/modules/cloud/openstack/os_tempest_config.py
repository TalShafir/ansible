...
options:
    ...
    deployer_input:
        description:
            ...
            The I(deployer_input) file is an alternative to providing I(overrides) key/value pairs.
    ...
    create:
        description:
            When C(True) create default tempest resources, require I(admin_cred) to be C(True)
        required: False
        default: 'False'
    admin_cred:
        description:
    ...
    use_test_accounts:
        description:
            path to accounts.yaml to be used if given
        required: False
        default: 'False'
    image_disk_format:
        description:
    ...
    remove:
        description: >
            Key values pairs to remove. 
            The format is a dictionary 'section.key : value[,value]' 
...
EXAMPLES = '''
- name: generate Tempest.conf without defaults or overrides
- os_tempest_config:
    dest: DEST

- name: generate Tempest.conf with defaults
- os_tempest_config:
    dest: DEST
    defaults_file: /path/to/file
    
- name: generate Tempest.conf with deployer-input and defaults
- os_tempest_config:
    dest: DEST
    defaults_file: /path/to/file
    deployer_input: /path/to/file
    
- name: generate Tempest.conf with overrides
- os_tempest_config:
    dest: DEST
    overrides:
...
- name: generate Tempest.conf and create Tempest resources(require I(admin_cred) to be C(True))
- os_tempest_config:
    dest: DEST
    admin_cred: True
    create: True
    
- name: generate Tempest.conf with accounts from accounts.yaml
- os_tempest_config:
    dest: DEST
    use_test_accounts: /path/to/accounts.yaml
...
- name: generate Tempest.conf with a supported image disk format
- os_tempest_config:
    dest: DEST
    image_disk_format: FORMAT

- name: generate Tempest.conf with an image to upload to glance
- os_tempest_config:
    dest: DEST
    image_disk_format: IMAGE
    
- name: generate Tempest.conf with a ID of a network's in our openstack instance with external connectivity
- os_tempest_config:
    dest: DEST
    network_id: ID
    
- name: generate Tempest.conf and save this script's log to the given file
- os_tempest_config:
    dest: DEST
    log_file: /path/to/log
   
- name: generate Tempest.conf without the specific key value pairs given
- os_tempest_config:
    dest: DEST
    remove:
        SECTION.KEY=VALUE[,VALUE]
        volume-feature-enabled.api_extensions: OS-SCH-HNT,os-hosts
...
RETURN = '''
dest:
    description: path to the tempest.conf file that was generated by this module
    returned: success
    type: string
    sample: "/path/to/tempest.conf"
'''
...
def main():
    ansible_module = AnsibleModule(argument_spec=dict(
        ...
        use_test_accounts=dict(type="path", required=False),
    ...
    try:
        ...
        if ansible_module.params["defaults_file"]:
            default_file_path = unfrackpath(ansible_module.params["defaults_file"])
            if os.path.isfile(default_file_path):
                LOG.info("Reading defaults from file '%s'", default_file_path)
                conf.read(default_file_path)

        if ansible_module.params["deployer_input"]:
            deployer_input_path = unfrackpath(ansible_module.params["deployer_input"])
            if os.path.isfile(deployer_input_path):
                LOG.info("Adding options from deployer-input file '%s'", deployer_input_path)

                deployer_input = ConfigParser.SafeConfigParser()
                deployer_input.read(deployer_input_path)
        ...
        if ansible_module.params["overrides"]:
            try:
                ...
                ansible_module.fail_json(
                    msg="couldn't parse overrides, make sure the overrides are a dict and that each key is in the format section.key, " + str(e))
        ...
        if ansible_module.params["use_test_accounts"]:
            ...
            conf.set("auth", "test_accounts_file", unfrackpath(ansible_module.params["use_test_accounts"]))
        ...
        ansible_module.fail_json(msg="Failed to generate tempest.conf, " + str(error))
